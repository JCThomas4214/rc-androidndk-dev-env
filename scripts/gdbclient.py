...
def do_main():
    ...
    root = os.path.join("mats", "ndk", "android-ndk-r20") #os.environ["ANDROID_BUILD_TOP"]
    sysroot = os.path.join("workspaces", "rc-androidndk-dev-env") #os.path.join(os.environ["ANDROID_PRODUCT_OUT"], "symbols")
    ...
    with binary_file:
        ...
        llvm_readobj_path = os.path.join(root, "prebuilts", "clang", "host",
                                         platform_name, "llvm-binutils-stable",
                                         "llvm-readobj")
        interp = gdbrunner.get_binary_interp(binary_file.name,
                                             llvm_readobj_path)
